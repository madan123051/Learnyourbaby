    ...
    return (
      <div className="p-4">
        <div className="text-center mb-6">
          <h2 className="text-2xl font-bold">{t.homeHeading}</h2>
          <p className="text-base-content/60 mt-1">{t.homeSubtitle}</p>
        </div>
            ...
            return (
              ...
              >
                <div className="card-body items-center text-center p-4">
                  ...
                  <p className="text-xs text-base-content/60">{t.catDesc[cat.name] ?? cat.name}</p>
                  <progress className="progress progress-primary w-full mt-1" value={learned} max={catWords.length} />
          ...
          <div className="card-body items-center text-center">
            ...
            {!flipped ? (
              <>
                ...
                <div className="badge badge-outline mt-3">{t.tapToFlip}</div>
              </>
            ) : (
              <div className="space-y-4 w-full">
                ...
                {isLearned && <div className="badge badge-success gap-1">{t.learned}</div>}
              </div>
            )}
          </div>
        </div>
      ...
      <div className="flex justify-between items-center mt-4 gap-2">
        ...
        <div className="flex gap-1">
          {categoryWords.map((_, i) => (
            <div key={i} className={`w-2 h-2 rounded-full ${i === cardIndex ? 'bg-primary' : 'bg-base-300'}`} />
          ))}
        </div>
